...
import WorkcardFrontendData from "./WorkcardFrontendData";
import WorkcardBackendData from "./WorkcardBackendData";
import WorkcardWordpressData from "./WorkcardWordpressData";
	...
	return (
		<div className={styles.works__container}>
			<div className={styles.works__containerBtnDiv}>
				<button className={clickFrontend ? "btn" : "btn btn__light"} onClick={handleClickFrontend}>
					Front-End
				</button>

				<button className={clickBackend ? "btn" : "btn btn__light"} onClick={handleClickBackend}>
					Back-End
				</button>

				<button className={clickWordpress ? "btn" : "btn btn__light"} onClick={handleClickWordpress}>
					Wordpress
				</button>
			</div>
			...
			<div className={styles.works__workcardContainer}>
				{clickFrontend &&
					WorkcardFrontendData.map((data, i) => {
						return (
							<Workcard key={i} imgsrc={data.imgsrc} title={data.title} siteUrl={data.siteUrl} githubUrl={data.githubUrl} />
						);
					})}
				...
				{clickWordpress &&
					WorkcardWordpressData.map((data, i) => {
						return <Workcard key={i} imgsrc={data.imgsrc} title={data.title} siteUrl={data.siteUrl} />;
					})}
			</div>
		</div>
	);
